    ...
    public class TCPClient_K
    {
        ...
        private void CommMethod()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    byte[] buff = MCRead(1000, 100);//1000개 바이트


                    PulseDetector getSign = new PulseDetector();


                    string data1 = buff[CalcByte(1000)].ToString();

                    string BCdata1 = buff[CalcByte(1005)].ToString();
                    string BCdata2 = buff[CalcByte(1006)].ToString();
                    string BCdata3 = buff[CalcByte(1007)].ToString();
                    string BCdata4 = buff[CalcByte(1008)].ToString();
                    string BCdata5 = buff[CalcByte(1009)].ToString();

                    string Vdata1 = (buff[CalcByte(1012)] + buff[CalcByte(1012) + 1] * 256).ToString();
                    string Vdata2 = (buff[CalcByte(1013)] + buff[CalcByte(1013) + 1] * 256).ToString();
                    ...
                    if (getSign.Detect(buff[CalcOffset(1003)], 1))      //1003번지에 1이 들어오면 
                    {
                        if (TalkingComm != null) TalkingComm("BarCode", "", BCdata1, BCdata2, BCdata3, BCdata4, BCdata5, Vdata1, Vdata2, "");
